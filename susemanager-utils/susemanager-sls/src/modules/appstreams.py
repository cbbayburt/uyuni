    ...
    if result.returncode == 0:
        try:
            ...
            if start_index is not None:
                ...
                print("Error: Unable to find module information in the output.")

        except (IndexError, ValueError) as e:
            print(f"Error parsing output: {e}")

    else:
        print(f"Error running DNF command: {result.stderr}")
...
def _parse_nsvca(module_info_output):
    ...
    for line in module_info_output:
        ...
        if match:
            ...
            if name and stream and version and context and architecture:
                break

    return f"{name}:{stream}:{version}:{context}:{architecture}" if name and stream and version and context and architecture else None
